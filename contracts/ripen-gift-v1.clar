...
(define-private (sbtc-transfer-in (amount uint) (from principal))
  (contract-call? .mock-sbtc-token transfer amount from (as-contract tx-sender) none))

(define-private (sbtc-pay-out (amount uint) (to principal))
  (as-contract (contract-call? .mock-sbtc-token transfer amount tx-sender to none)))
